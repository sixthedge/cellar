module Thinkspace; module ReadinessAssurance; module Reconcilers
  class Assignment
    ...
    def update_responses
      get_responses.each do |response|
        @delta.each do |change|
          id = change[:id]
          if change[:new]
            response.user_data[question_scores_key][id] = 0 if response.user_data[question_scores_key].present?
            response.user_data[question_correct_key][id] = false if response.user_data[question_correct_key].present?
    ...
    def update_response_data_for_question(response, id, from_key, to_key, deleted)
      ...
      data[question_scores_key].delete(id)   if deleted
      data[question_correct_key][id] = false if data[question_correct_key].present?
      data[question_correct_key].delete(id)  if deleted
    ...
    def get_delta
      ...
      get_questions.each do |question|
        ...
        if t_question.present?
          delta << {id: question['id'], new: false, deleted: false, dirty: get_question_is_dirty(question['id'])}
        else
    ...
    def get_correct_answers; @assessment.answers; end
    def get_correct_transform_answers; @transform[answers_key]; end
