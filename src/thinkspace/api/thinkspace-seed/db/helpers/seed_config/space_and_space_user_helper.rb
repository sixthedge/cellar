...
def casespace_seed_config_add_space_users(config)
  ...
  seed_config_message('++Adding seed config space users.', config)
  space_users.each do |hash|
    ...
    spaces.each do |title|
      space = find_casespace_space(title: title)
      seed_config_error "Space users space #{title.inspect} not found [space_users: #{hash.inspect}].", config  if space.blank?
...
def casespace_seed_config_add_repeat_space_users(config)
  ...
  return if space_users.blank?
  seed_config_message('++Adding seed config "repeat" space users.', config)
  space_users.each do |hash|
