...
def casespace_seed_config_add_teams(config)
  ...
  when teams.is_a?(Array)
    teams.each do |teams_hash|
      seed_config_error "Teams is not a hash #{teams_hash.inspect}.", config unless teams_hash.is_a?(Hash)
      casespace_seed_config_add_teams_in_hash(config, teams_hash)
    end
  else
    seed_config_error "Teams must be a hash or array of hashes.", config
  end
end

def casespace_seed_config_add_teams_in_hash(config, teams_hash)
  seed_config_models.set_find_by(config)
  seed_config_message('++Adding seed config teams.', config)
...
def casespace_seed_config_add_team_sets(config, teams_hash)
  ...
  team_sets_hash.each_with_index do |hash, index|
    ...
    if title.present?
      ...
      seed_config_error "Space title #{title.inspect} not found." if space.blank?
    end
    seed_config_error "Team set must define a space or be inherited [#{hash.inspect}]." if space.blank?
    ...
    seed_config_error "Team set #{title.inspect} already exists for space #{hash[:space].inspect}"  if team_set.present?
...
def casespace_seed_config_add_team_set_teams(config, teams_hash)
  ...
  team_set_teams_hash.each_with_index do |hash, index|
    ...
    if title.present?
      team_set = casespace_seed_config_find_team_team_set(space_id: space.id, title: title)
      seed_config_error "Team set #{title.inspect} for space #{space.title.inspect} not found." if team_set.blank?
    end
    ...
    if team.present? && casespace_dup_skip?(hash)
      seed_config_models.add(config, team)
      next
    end
    description = hash[:description] || "description for #{title}"
    state       = hash[:state]
...
def casespace_seed_config_add_assignment_teams(teams_hash)
  ...
  assignment_teams.each_with_index do |hash, index|
    ...
    if title.present?
      space = find_casespace_space(title: title)
      seed_config_error "Assignment teams space title #{title.inspect} not found." if space.blank?
    end
    seed_config_error "Assignment team space has not been specified and is not inheritable [#{hash.inspect}]."  if space.blank?
    title = hash[:title]
    seed_config_error "Assignment team title is blank [#{hash.inspect}]." if title.blank?
    assignment = find_casespace_assignment(space_id: space.id, title: title)
    seed_config_error "Assignment title #{title.inspect} for space #{space.title.inspect} not found [#{hash.inspect}]." if assignment.blank?
...
def casespace_seed_config_add_phase_teams(teams_hash)
  ...
  phase_teams.each_with_index do |hash, index|
    ...
    if title.present?
      space = find_casespace_space(title: title)
      seed_config_error "Phase team space #{title.inspect} not found [#{hash.inspect}]."  if space.blank?
    end
    seed_config_error "Phase team space has not been specified and is not inheritable [#{hash.inspect}]."  if space.blank?
    ...
    if title.present?
      ...
      seed_config_error "Phase team assignment #{title.inspect} not found [#{hash.inspect}]."  if assignment.blank?
    else
      seed_config_error "Phase team assignment has not been specified and is not inheritable [#{hash.inspect}]."  if assignment.blank?
    end
    title = hash[:title]
    seed_config_error "Phase title is blank for phase team [#{hash.inspect}]."  if title.blank?
    phase = find_casespace_phase(assignment_id: assignment.id, title: title)
    seed_config_error "Phase #{title.inspect} for phase team not found [#{hash.inspect}]."  if phase.blank?
