...
#     observations: {}
# would call:
#   * AutoInputResponses.new(self, @seed, casespace_config_models, options)
#   * AutoInputObservations.new(self, @seed, casespace_config_models, options)
# The class .rb files should be in the 'test_data/auto_input' folder (but can be in any required .rb).

def casespace_seed_config_add_auto_input(config)
  auto_input = config[:auto_input]
  return if auto_input.blank?
  @seed.message color_line("--Auto input (#{casespace_seed_config_name(config)}).", :green, :bold)
  auto_input.keys.each do |key|
    class_name = "AutoInput#{key.to_s.camelize}"
    klass      = class_name.safe_constantize
    casespace_seed_config_error "Auto input class #{class_name.inspect} does not exist.  Is it in the auto_input folder?."  if klass.blank?
    casespace_seed_config_add_auto_input_array(auto_input, key, config).each do |hash|
      ...
      klass.new(self, @seed, casespace_config_models, config, hash)
    end
  end
end
...
def casespace_seed_config_add_auto_input_config_only_phases(hashes, config)
  ...
  hashes.each do |hash|
    ...
    if only.present?
      ...
      casespace_seed_config_error "Auto input options[:only] phase titles missing #{missing_titles}.\n  Options: #{hash.inspect}.", config  if missing_titles.present?
