...
def casespace_seed_config_add_weather_forecasters(config)
  ...
  casespace_seed_config_message('++Adding seed config weather forecaster assessments.', config)
...
def create_casespace_weather_forecaster_assessments(config, wf_hash)
  ...
  assessments.each do |hash|
    ...
    if title.present?
      ...
      casespace_seed_config_error "Weather forecast assignment #{title.inspect} not found [#{hash.inspect}].", config  if assignment.blank?
    else
      casespace_seed_config_error "Weather forecast assignment has not been specified and is not inheritable [#{hash.inspect}].", config  if assignment.blank?
    end
    title = hash[:phase]
    if title.present?
      phase = find_casespace_phase(title: title, assignment_id: assignment.id)
      casespace_seed_config_error "Weather forecast phase #{title.inspect} not found [#{hash.inspect}].", config  if phase.blank?
    else
      casespace_seed_config_error "Weather forecast phase has not been specified and is not inheritable [#{hash.inspect}].", config  if phase.blank?
    end
    ...
    if item_names.present?
      ...
      [item_names].flatten.compact.each do |name|
        ...
        casespace_seed_config_error "Weather forecast item #{name.inspect} not found [#{hash.inspect}].", config  if item.blank?
        items.push item
      end
    else
      casespace_seed_config_error "Weather forecast items have not been specified and is not inheritable [#{hash.inspect}].", config  if items.blank?
    end
    ...
    if station_code.present?
      ...
      casespace_seed_config_error "Weather forecast station #{station_code.inspect} not found [#{hash.inspect}].", config  if station.blank?
    else
      casespace_seed_config_error "Weather forecast station has not been specified and is not inheritable [#{hash.inspect}].", config  if station.blank?
    end
    title = hash[:title]
    casespace_seed_config_error "Weather forecast assessment title is blank [#{hash.inspect}].", config  if title.blank?
