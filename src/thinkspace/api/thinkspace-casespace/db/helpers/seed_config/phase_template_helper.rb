...
def casespace_seed_config_add_phase_templates(config)
  ...
  casespace_seed_config_message('++Adding seed config phase templates.', config)
  templates.each do |hash|
    template = hash[:template]
    casespace_seed_config_error "Phase template does not have a template value [template: #{hash.inspect}].", config  if template.blank?
