...
def casespace_seed_config_add_assignments(config)
  ...
  casespace_seed_config_message('++Adding seed config assignments.', config)
  space = nil
  assignments.each do |hash|
    title = hash[:space]
    if title.present?
      space = find_casespace_space(title: title)
      casespace_seed_config_error "Assignment space #{title.inspect} not found [assignment: #{hash.inspect}].", config  if space.blank?
    else
      casespace_seed_config_error "Space assignment has not been specified or is inheritable [assignment: #{hash.inspect}].", config  if space.blank?
    end
...
def casespace_seed_config_add_phases(config)
  ...
  casespace_seed_config_message('++Adding seed config phases.', config)
  space      = nil
  assignment = nil
  template   = nil
  phases.each do |hash|
    ...
    if title.present?
      space = find_casespace_space(title: title)
      casespace_seed_config_error "Phase space #{title.inspect} not found [phase: #{hash.inspect}].", config  if space.blank?
    end
    ...
    if title.present?
      ...
      casespace_seed_config_error "Phase assignment #{title.inspect} not found [phase: #{hash.inspect}].", config  if assignment.blank?
    else
      casespace_seed_config_error "Phase assignment has not been specified and is not inheritable [phase: #{hash.inspect}].", config  if assignment.blank?
    end
    template_name = hash[:template_name]
    if template_name.present?
      template = find_casespace_phase_template(name: template_name)
      casespace_seed_config_error "Phase template name #{template_name.inspect} not found [phase: #{hash.inspect}].", config  if template.blank?
    else
      casespace_seed_config_error "Phase template has not be specified and is not inheritable [phase: #{hash.inspect}].", config  if template.blank?
    end
    ...
    if (category = hash[:team_category]).present?
      ...
      casespace_seed_config_error "Phase team category #{category.inspect} not found [phase: #{hash.inspect}].", config  if team_category.blank?
    else
      team_category = nil
    end
