class SocketIOTimerCountdown
  ...
  process: ->
    ...
    unless @valid_and_values_set()
      @timer.remove(@)
      return
    @util.debug @timer_message('added. id:'), @id
    @final   = false
    @n       = 1
    @running = 0
    if @start_at
  ...
  setup_timeout: ->
    @n       += 1
    @running += @inc
    @timeout  = if @running > @total_timeout then ( @total_timeout - (@running - @inc) ) else @inc
    @set_units()
    @units = 0 if @units < 0
    @set_final()

  set_final: ->
    @final = true if @units <= 0
  ...
  set_units:            -> @units            = Math.floor( (@total_timeout - @running) / @unit_ms)
  set_number_of_timers: -> @number_of_timers = Math.ceil(@total_timeout / @inc) + 1

  valid_and_values_set: ->
    unless @end_at
      @util.warn @timer_message('end_at is blank.'), {@id, @cid, @data}
      return false
    if @start_at and @start_at > @end_at
      @util.error @timer_message("start at '#{@start_at}' greater than end_at '#{@end_at}'."), {@id, @cid, @data}
      return false
    @set_total_timeout()
    unless @total_timeout and @total_timeout > 0
      @util.warn @timer_message('in the past. Not running timer.'), {@id}
      return false
    @set_number_of_timers()
    if @max and @number_of_timers > @max
      @util.error @timer_message("has over '#{@max}' timers for id '#{@id}'.  Would have created '#{@number_of_timers}'."), {@id, @cid, @data}
      return false
    true

  timer_message: (msg='') -> @util.bold_line("TIMER #{@type} ", 'magenta') + msg

  debug: -> @util.debugging and @helpers.debug(@)

  to_string: -> 'SocketIOTimerCountdown'

module.exports = SocketIOTimerCountdown
